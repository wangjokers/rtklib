                  ...
                  const prcopt_t *opt, int *sat, int *iu, int *ir)
{
    ...
    for (i=0,j=nu;i<nu&&j<nu+nr;i++,j++) {//双指针，一个指向基准站，一个指向流动站
        if      (obs[i].sat<obs[j].sat) j--;
        else if (obs[i].sat>obs[j].sat) i--;//这两步进行对齐
        else if (azel[1+j*2]>=opt->elmin) { /* elevation at base station *///截止高度角
            sat[k]=obs[i].sat; iu[k]=i; ir[k++]=j;//sat存储的是卫星共有的卫星编号，iu和ir存储对应的索引
...
static void udpos(rtk_t *rtk, double tt)
{
    ...
    if (rtk->opt.mode==PMODE_FIXED) {
        for (i=0;i<3;i++) initx(rtk,rtk->opt.ru[i],1E-8,i);//ru是输入值，权重很大
        return;
    }
    /* initialize position for first epoch *///给历元初值，
    if (norm(rtk->x,3)<=0.0) 
    {
    ...
    if (!rtk->opt.dynamics) {
        for (i=0;i<3;i++) initx(rtk,rtk->sol.rr[i],VAR_POS,i);//用单点的结果来填充
        return;
    }
    ...
    if (var>VAR_POS) {
        /* reset position with large variance 太大的话重置，用单点的值*/
        for (i=0;i<3;i++) initx(rtk,rtk->sol.rr[i],VAR_POS,i);
        for (i=3;i<6;i++) initx(rtk,rtk->sol.rr[i],VAR_VEL,i);
        for (i=6;i<9;i++) initx(rtk,1E-6,VAR_ACC,i);
    ...
    /* process noise added to only acceleration这里的过程噪声增加到了加速度上 ，接着会传递到速度和位置上*/
    Q[0]=Q[4]=SQR(rtk->opt.prn[3])*fabs(tt);
    Q[8]=SQR(rtk->opt.prn[4])*fabs(tt);//水平的
    ecef2pos(rtk->x,pos);
    covecef(pos,Q,Qv);
...
static void udtrop(rtk_t *rtk, double tt, double bl)
{
    ...
    for (i=0;i<2;i++) {
        ...
        if (rtk->x[j]==0.0) {
            ...
            rtk->P[j+j*rtk->nx]+=SQR(rtk->opt.prn[2])*fabs(tt);//只需要加一个过程噪声方法一下就可以了
                    ...
                    const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    /* temporal update of eceiver h/w bias 通道时延*/
    if (rtk->opt.glomodear==2&&(rtk->opt.navsys&SYS_GLO)) {
        udrcvbias(rtk,tt);
    }
    /* temporal update of phase-bias 模糊度 */
                 ...
                 int index, double *y, double *e, double *azel, double *freq)
{
    ...
    for (i=0;i<3;i++) rr_[i]=rr[i];//传入的是const，不可以修改，所以要进行赋值的操作
    
    /* earth tide correction *///rtk中因为离得比较近，所以可以认为不改这项
    if (opt->tidecorr) {
    ...
    for (i=0;i<n;i++) {
        /* compute geometric-range and azimuth/elevation angle *///使用到的多频，
    ...
    for (i=0;i<n;i++) {
        trace(4,"sat=%2d %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n",
              ...
              azel[1+i*2]*R2D);//这些和计算卫星的位置息息相关，可以检查错误
    }
...
static int ddres(rtk_t *rtk, const nav_t *nav, double dt, const double *x,
    ...
    //后面的iu和ir的意思是共视卫星里面流动站的index在obs里头，通过index可以计算出残差在每个index的位置。
{
    ...
    bl=baseline(x,rtk->rb,dr);//计算了基线长度
    ecef2pos(x,posu); ecef2pos(rtk->rb,posr);
    
    Ri=mat(ns*nf*2+2,1); Rj=mat(ns*nf*2+2,1); im=mat(ns,1);//代表基准站和流动站的方差
    ...
    for (i=0;i<ns;i++) {
        ...
        }//长基线就需要估计这些值，一般不会涉及长基线
    }
    for (m=0;m<6;m++) /* m=0:GPS/SBS,1:GLO,2:GAL,3:BDS,4:QZS,5:IRN */
    
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {//对指定系统指定频率进行处理
        
        /* search reference satellite with highest elevation *///可以修改选择的卫星，选择了截止高度角最高的
        for (i=-1,j=0;j<ns;j++) {
        ...
        for (j=0;j<ns;j++) {//遍历所有卫星
            ...
            /* DD phase-bias term *///模糊度，双差的载波偏差
            if (f<nf) {//遍历载波
                if (opt->ionoopt!=IONOOPT_IFLC) {//无电离层组合，一般比较费劲
            ...
            if (f<nf) rtk->ssat[sat[j]-1].resc[f   ]=v[nv];//残差放回结构体里面
            ...
            /* SD (single-differenced) measurement error variances *///单差的方式构造
            ...
            //参考站左移16位，用一些列的于或操作来代表残差是哪一个卫星，哪一个频率的值
            nb[b]++;//每次切换频率就+1
        }
        b++;
    }
                  ...
                  int nv, double thres)
{
    ...
    for (i=0;i<nv;i++) {
        ...
        sat1=(vflg[i]>>16)&0xFF;//高八位参考星的卫星编号
        sat2=(vflg[i]>> 8)&0xFF;//被差星的卫星编号
        type=(vflg[i]>> 4)&0xF;//标记载波还是伪距的残差
        freq=vflg[i]&0xF;      //频率索引区分不同的频段
                  ...
                  const nav_t *nav)
{
    ...
    rs=mat(6,n); dts=mat(2,n); var=mat(1,n); y=mat(nf*2,n); e=mat(3,n);//为什么乘2是因为有伪距和载波
    ...
    /* UD (undifferenced) residuals for base station *///把有文星的提出掉，基准站用的是基准站的正确坐标
    if (!zdres(1,obs+nu,nr,rs+nu*6,dts+nu*2,var+nu,svh+nu,nav,rtk->rb,opt,1,//计算的是基准站的，所以后面都加上nu，感兴趣的量
    ...
    /*ns是公共的卫星个数，nu基站站的个数，nr流动站的个数，obs是基站站和流动站的观测数据
    sat是公共卫星号结算出来，*/
    ...
    /* temporal update of states */
    udstate(rtk,obs,sat,iu,ir,ns,nav);//时间状态更新，预测状态和协方差
    
    trace(4,"x(0)="); tracemat(4,rtk->x,1,NR(opt),13,4);
    
    xp=mat(rtk->nx,1); Pp=zeros(rtk->nx,rtk->nx); xa=mat(rtk->nx,1);
    matcpy(xp,rtk->x,rtk->nx,1);//nx存储的是浮点状态量的个数
    
    ny=ns*nf*2+2;
    v=mat(ny,1); H=zeros(rtk->nx,ny); R=mat(ny,ny); bias=mat(rtk->nx,1);//跟ppp的卡尔曼滤波构造有点像
    //通过星间和站间双差
    /* add 2 iterations for baseline-constraint moving-base */
    niter=opt->niter+(opt->mode==PMODE_MOVEB&&opt->baseline[0]>0.0?2:0);
    
    for (i=0;i<niter;i++) {
        /* UD (undifferenced) residuals for rover *///流动站则用流动站的概略坐标
    ...
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,var,svh,nav,xp,opt,0,y,e,azel,
        ...
        //滤波前后都可以去剔除卫星质量不好的卫星，修正残差
        /* validation of float solution *///验证浮点解
...
{
    prcopt_t *opt=&rtk->opt;   //取结构体成员地址赋值给指针opt
    sol_t solb={{0}};
    gtime_t time;
    int i,nu,nr;
    char msg[128]="";
    ...
    if (opt->refpos<=POSOPT_RINEX&&opt->mode!=PMODE_SINGLE&&
        ...
        for (i=0;i<6;i++) rtk->rb[i]=i<3?opt->rb[i]:0.0;//起始值全置为0
    }
