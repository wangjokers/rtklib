...
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    ...
    for (i=0;i<2;i++) {
        t=ts-(eph->f0+eph->f1*t+eph->f2*t*t);//钟偏的多项式系数,课本75页,文档 见(E.4.16)
    }//使用二项式校正得到最终的钟差
                    ...
                    double *var)
{
    ...
    tk=timediff(time,eph->toe);//计算规一化时间 tk (E.4.2)
    ...
    if (sys==SYS_CMP&&(prn<=5||prn>=59)) { /* ref [9] table 4-1 */
        ...
        rs[0]= xg*coso+yg*sino*COS_5+zg*sino*SIN_5;//北斗的IGSO和MEO与GPS坐标计算相同
        rs[1]=-xg*sino+yg*coso*COS_5+zg*coso*SIN_5;//而在ECEF系统中，GEO卫星的轨道倾角为零
    ...
    *dts=eph->f0+eph->f1*tk+eph->f2*tk*tk;//求最优解下的卫星钟差
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    for (i=0;i<nav->n;i++) { //遍历所有星历表；北斗和GPS对于iode的定义不同
...
static int ephclk(gtime_t time, gtime_t teph, int sat, const nav_t *nav,
                  double *dts)
{   /*gtime_t time: 用户指定的时间，表示需要计算钟差的时间。
    gtime_t teph: 星历参考时间，通常是星历数据中提供的时间。
    int sat: 卫星编号，用于指定特定的卫星。
    ...
    */
    ...
    sys=satsys(sat,NULL); //调用 satsys 函数，根据卫星编号确定该卫星所属的导航系统和该卫星在该系统中的 PRN编号
    
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        if (!(eph=seleph(teph,sat,-1,nav))) //调用 seleph 函数来选择最接近 teph 的那个星
                return 0;//至此星历匹配成功
        *dts=eph2clk(time,eph);//调用 eph2clk 函数，通过广播星历和信号发射时间计算出卫星钟差，北斗的可以用这个算吗？
    }
    else if (sys==SYS_GLO) {
...
{/*gtime_t time: 观测时间（接收机时间）。
gtime_t teph: 星历参考时间，通常是星历提供的时间。
int sat: 卫星编号，用于标识特定卫星。
const nav_t *nav: 包含所有卫星星历数据的结构体。
int iode: 星历的 IODE 参数，用于识别特定星历信息。
double *rs: 输出参数，用于存储卫星的位置和速度（共 6 个元素，前 3 个是位置，后 3 个是速度）。
double *dts: 输出参数，用于存储卫星的钟差（dts[0]）和钟漂（dts[1]）。
double *var: 输出参数，存储卫星位置的方差，用于衡量位置的精度。
int *svh: 输出参数，存储卫星健康状态。*/
    eph_t  *eph;
    geph_t *geph;
    seph_t *seph;
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP||sys==SYS_IRN) {
        ...
        time=timeadd(time,tt);//认为加入了一个tt，得到卫星的新位置和新钟差（通过扰动法来计算出卫星的速度和钟漂，注意不是接收机），静态用不到
...
*          nav_t  *nav      I   navigation data
*          int    ephopt    I   ephemeris option (EPHOPT_???)
*          double *rs       O   satellite positions and velocities (ecef)   长度为6*n，{x,y,z,vx,vy,vz}(ecef)(m,m/s)
*          double *dts      O   satellite clocks
*          double *var      O   sat position and clock error variances (m^2)  卫星位置和钟差的协方差
*          int    *svh      O   sat health flag (-1:correction not available)
* return : none
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        //跳到下一个卫星，北斗可以有四个频点，可能出错，写得不稳健
        /* transmission time by satellite clock 得到发射时间*/
        ...
        //利用广播星历矫正发射时间，利用发射时间求位置和速度等信息
        //rs的每一列有六个元素，dts每一列有两个元素分别为钟差和钟速
