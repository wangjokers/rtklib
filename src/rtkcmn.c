...
*-----------------------------------------------------------------------------*/
//据卫星系统（sys）和PRN号（Pseudo-Random Noise Number，伪随机噪声码号）计算并返回一个统一的卫星编号（satellite number）
...
extern int getcodepri(int sys, uint8_t code, const char *opt)
{
    ...
    p= strchr(codepris[i][j]，obs[1]);//i=5:BDS j:采用codepris[]第i行的第j个优先级排序*/
...
* 创建新矩阵，传入行数 n，列数 m，malloc 开辟 n*m 个double空间，返回此空间的首地址。
*-----------------------------------------------------------------------------*/
...
* return : matrix pointer (if n<=0, return NULL)    单位矩阵
*-----------------------------------------------------------------------------*/
extern double *eye(int n)
{
    ...
    if ((p=zeros(n,n))) for (i=0;i<n;i++) p[i+i*n]=1.0;//取对角线的值赋值一
    return p;
}
/* inner product ---------------------------------------------------------------
...
/*
返回:C=alpha*A*B+beta*C  其中的
alpha&beta是数，A、B是相乘的2个矩阵 tr为是否转置的标志
n,k,m表示两个矩阵的大小（两个矩阵相乘，前矩阵的列=后矩阵的行），故三个参数足矣 )
n:第一个矩阵的行；m：第一个矩阵的列/第二个矩阵的行;k:第二个矩阵的列，
作者之所以把n和k放在m前面，其实是矩阵相乘一共要循环n*k次，从编程的顺序来排的
*/
/* multiply matrix -----------------------------------------------------------*/
extern void matmul(const char *tr, int n, int k, int m, double alpha,
                   const double *A, const double *B, double beta, double *C)
{
    ...
    for (i=0;i<n;i++) for (j=0;j<k;j++) {
        d=0.0;
        switch (f) {
            case 1: for (x=0;x<m;x++) d+=A[i+x*n]*B[x+j*m]; break;//A*B,注意矩阵的脚标是竖着排的，这里是左行*右列
            case 2: for (x=0;x<m;x++) d+=A[i+x*n]*B[j+x*k]; break;//左行*右行，A*B'
            case 3: for (x=0;x<m;x++) d+=A[x+i*m]*B[x+j*m]; break;//左列*右列，A'*B
            case 4: for (x=0;x<m;x++) d+=A[x+i*m]*B[j+x*k]; break;//左行*右行，A'*B'
        }
        if (beta==0.0) C[i+j*n]=alpha*d; else C[i+j*n]=alpha*d+beta*C[i+j*n];
    }
}
/* LU decomposition ---------数值分析有涉及-------------------------------------------------*/
...
/* solve linear equation ----求线性方程组的解 、AX=Y，求 X，A 为 nn，Y为 nm，用的不多，模糊度固定里涉及到了。-------------------------------------------------*/
extern int solve(const char *tr, const double *A, const double *Y, int n,
                 int m, double *X)
{
    double *B=mat(n,n);
...
*          double *v        I   innovation (m  easurement - model) (m x 1)
                  ...
                  const double *R, int n, int m)
{
    ...
    int i,j,k,info,*ix;
    //x阵中有大量的空数据0，基本都在模糊度那里，去掉0数据，精简x阵，否则高阶矩阵求逆等会出问题
...
{//打印矩阵到终端控制台
    matfprint(A,n,m,p,q,stdout);//标准控制台
}
/* string to number ------------------------------------------------------------
                       ...
                       const double *azel)
{
    ...
    psi=0.0137/(azel[1]/PI+0.11)-0.022;  //aze[0]表示方位角，aze[1]表示仰角，这里的方位角是角度的，转换成半圆要除以π
    
    /* subionospheric latitude/longitude (semi-circle) */
    phi=pos[0]/PI+psi*cos(azel[0]);      //pos[0]，代表接收机的纬度，1代表经度
