...
static nav_t navs={0};          /* navigation data ，存储钟差，精密星历等文件，*/
                    ...
                    int mode)
{
    ...
    sol_t sol={{0}};
    rtk_t rtk;
    obsd_t obs[MAXOBS*2]; /* for rover and base */ //一开始就是192个元素
    double rb[3]={0};
    ...
    while ((nobs=inputobs(obs,rtk.sol.stat,popt))>=0) {//读取o文件下一个历元所有卫星的数据（所有系统的卫星都读）
        ...
        /* exclude satellites */
        for (i=n=0;i<nobs;i++) {
            if ((satsys(obs[i].sat,NULL)&popt->navsys)&&  //根据设置的卫星系统和废弃卫星编号剔除无用obs
                popt->exsats[obs[i].sat-1]!=1) obs[n++]=obs[i];  //比如星历以供观测了54个卫星，其中有十个是北斗的，则此步会把obs[0]至obs[9]全部用来存这十个北斗
                        ...
                        nav_t *nav, sbs_t *sbs)
{
    ...
    nav->ne=nav->nemax=0;//清除精密星历数据
    nav->nc=nav->ncmax=0;//清除精密轨道数据
                   ...
                   const filopt_t *fopt, nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
    ...
    if (*fopt->satantp&&!(readpcv(fopt->satantp,pcvs))) {//路径非空且读取失败就报错
        ...
        for (i=0;;i++) { /* for each periods */
            ...
            for (j=k=nf=0;j<n;j++) {  //遍历infile[]，根据后缀名
                ...
                if (ext&&(!strcmp(ext,".rtcm3")||!strcmp(ext,".RTCM3"))) {
                    strcpy(ifile[nf++],infile[j]);// 如果是 RTCM 文件，直接复制到 ifile 中
                }
                    ...
                    if (ext&&(!strcmp(ext,".sp3")||!strcmp(ext,".SP3")||
                        ...
                        ttte=timeadd(ttte,3600.0); // 精密星历文件，多包含一小时
                    }
                    else if (strstr(infile[j],"brdc")) {
                        ttte=timeadd(ttte,7200.0);// 广播星历文件，多包含两小时
                    }
