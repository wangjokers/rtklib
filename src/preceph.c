    ...
    for (i=0;;i++) {
        ...
        if (i==0) {
            *type=buff[2];
            ...
            if (ns==0) {
                ns=(int)str2num(buff,3,3);//修改了对应长度，原本一个4和2，读取的sp3文件头的卫星数会缺少
            }
            for (j=0;j<17&&k<ns;j++) {//这里因为每一行格式限制标准最多存储17颗卫星
           ...
           /* strncpy(tsys,buff+9,3); tsys[3]='\0';*/
            ...
            fseek(fp,-(long)sizeof(buff),SEEK_CUR);
            break;
        }
    }
    return ns;
...
由精密星历求解卫星位置+钟差(15min)(PPP的钟差由c1k文件提供 30s)
当未提供c1k文件时，本函数求得的钟差作为精密钟差投入后续计算)
time:发射信号时的卫星钟面时刻
vare:卫星位置的方差
varc:卫星钟差的方差
-----------------------------------*/
                   ...
                   double *dts, double *vare, double *varc)
{
    ...
    //当差值节点<差值阶数+1||信号播发时刻不在sp3文件起止时间范围内，跳
    if (nav->ne<NMAX+1||
    ...
    /* binary search  二分查找搜索距离卫星信号播发时刻最近的星历数组指针 */
    ...
    for (j=0;j<=NMAX;j++) {
        t[j]=timediff(nav->peph[i+j].time,time);//上五行：eg，idx=6,10阶插值，则取1-11历元的星历进行插值求解
    ...
    for (j=0;j<=NMAX;j++) {
        ...
        /* 地球自传改正correciton for earh rotation ver.2.4.0 */
        sinl=sin(OMGE*t[j]);
        cosl=cos(OMGE*t[j]);
        p[0][j]=cosl*pos[0]-sinl*pos[1];//x
        p[1][j]=sinl*pos[0]+cosl*pos[1];//y
        p[2][j]=pos[2];//z卫星坐标为ecef，故z轴无需改正
    }
    //开始插值
    for (i=0;i<3;i++) {
                    ...
                    double *rs, double *dts, double *var)
{
    ...
    /* relativistic effect correction 相对论改正*/
    if (dtss[0]!=0.0) {
    ...
    if (var) *var=vare+varc;//合并卫星位置方差和钟差
