...
#define P2_57       1.734723475976807E-17 /* 2^-57 */
...
typedef struct {        /* observation data record */
    ...
    uint8_t code[NFREQ+NEXOBS]; /* code indicator (CODE_???)观测值类型eg，L2I  L5P */
    double L[NFREQ+NEXOBS]; /* observation data carrier-phase (cycle) */
    double P[NFREQ+NEXOBS]; /* observation data pseudorange (m) */
    float  D[NFREQ+NEXOBS]; /* observation data doppler frequency (Hz) */
} obsd_t;//(故obsd_t obs;则obs[0].time=obs[i].time=obs[end].time=当前历元)
...
typedef struct {        /* navigation data type */
    ...
    double cbias[MAXSAT][MAX_CODE_BIAS_FREQS][MAX_CODE_BIASES]; /* satellite DCB (0:P1-P2,1:P1-C1,2:P2-C2) (m)这里的序号排列不太确定？ */
...
typedef struct {        /* solution type */
    ...
    double dtr[6];      /* receiver clock bias to time systems (s) 单系统仅dtr[0]有用，即接收机钟差*/
...
typedef struct {        /* file options type */
    char satantp[MAXSTRPATH]; /* satellite antenna parameters file */   //卫星天线文件
    char rcvantp[MAXSTRPATH]; /* receiver antenna parameters file */        
    char stapos [MAXSTRPATH]; /* station positions file */
    char geoid  [MAXSTRPATH]; /* external geoid data file */    //外部水准面文件
...
typedef struct {        /* RTK control/result type */
    ...
    double rb[6];       /* base position/velocity (ecef) (m|m/s)基准站 */
    int nx,na;          /* number of float states/fixed states 模糊度浮点解和固定解模式*/
