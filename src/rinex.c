...
//去除结尾空格，传入字符串src，返回去除结尾空格的字符串dst。
...
//调整时间差到半周内，传入两个时间，算时间差，通过加减整周秒数，将时间差调整到半周内。
...
/* adjust time considering week handover -------------------------------------*/
//调整时间差到半天内，传入两个时间，算时间差，通过加减整周秒数，将时间差调整到半周内。
...
static double uravalue(int sva) //用户距离精度 URA 到名义值，用于计算卫星位置的时候标定方差。
{
    return 0<=sva&&sva<15?ura_nominal[sva]:8192.0;
}
/* URA value (m) to URA index ------------------------------------------------*/
static int uraindex(double value)   //URA 值到 URA 下标，
{
    int i;
...
static double sisa_value(int sisa)  //Galileo卫星系统完备性SISA下标到值
{
...
static int sisa_index(double value) //Galileo卫星系统完备性SISA值到下标
{
                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    //用来检查字符串 label（文件头的一部分）是否包含 "MARKER NAME"。如果找到这个标签，程序就执行相应的逻辑，比如提取站点名称。
    ...
    else if (strstr(label,"SYS / PHASE SHIFTS"  )) ; /* ver.3.01 *///空指令，不读取不进行操作
...
static void decode_navh(char *buff, nav_t *nav)
{
    ...
    trace(4,"decode_navh:\n");  //读取电离层模型参数，用于记录解码过程的日志（trace函数一般用于调试）。
    
    if      (strstr(label,"ION ALPHA"           )) { /* opt ver.2 */    //电离层模型参数的四个a和b系数
        if (nav) {
    ...
    }//在检测到"DELTA-UTC: A0,A1,T,W"时，函数提取UTC修正参数A0、A1（用于线性修正）、参考时间T和GPS周W。
    //这些参数用来将GPS系统时间转化为UTC时间，储存在nav->utc_gps数组中。
        ...
        }//判断不同的卫星系统标识（GPS、Galileo、BDS、QZSS等），并解析相应的电离层修正参数
         //存储在对应的结构体数组中，如nav->ion_gps、nav->ion_gal、nav->ion_cmp等。
    }//读取时间系统改正参数
    else if (strstr(label,"TIME SYSTEM CORR"    )) { /* opt ver.3 */
        if (nav) {
            if (!strncmp(buff,"GPUT",4)) {
                nav->utc_gps[0]=str2num(buff, 5,17);//从buff（电文缓冲区）位置5开始，提取长为17的字符串转换为数值
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    for (i=0,j=ver<=2.99?0:3;i<ind->n;i++,j+=16) {
        ...
        if (stat) {
            val[i]=str2num(buff,j,14)+ind->shift[i];//eg有18个观测值类型，则va1[18]记录了这18个观测数值(伪距观测、相位观测·.....)
    ...
    for (i=n=m=0;i<ind->n;i++) {
        ...
        if (ind->type[i]==0&&p[i]==0) k[n++]=i; /* C1? index *//* 记录首选频率的伪距观测在所有观测类型中的下标位置*/
        if (ind->type[i]==0&&p[i]==1) l[m++]=i; /* C2? index *//* 记录次选频率的伪距观测在所有观测类型中的下标位置*/
    }//m、n通常为1
    if (ver<=2.11) {
    ...
    for (i=0;i<ind->n;i++) {
        if (p[i]<0||val[i]==0.0) continue;//p[i]:确保是所选3频;va1[i]:确保该观测值不为空
        switch (ind->type[i]) {
            case 0: obs->P[p[i]]=val[i]; obs->code[p[i]]=ind->code[i]; break;//p[i]=0 1 2分别对应首选、次选、次次选频率
...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
    ...
    if (obs->nmax<=obs->n) {    //obs->n
        if (obs->nmax<=0) obs->nmax=NINCOBS; else obs->nmax*=2;     //realloc是对之前的malloc分配的内存进行重新分配，在设置的nmax值小于实际n值时执行
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (i=n=0;*tobs[i];i++,n++) {
        ...
        ind->type[i]=(p=strchr(obscodes,tobs[i][0]))?(int)(p-obscodes):0;//0:C,1:L,2:D,3:S,判断观测值种类是否存在，存在则赋值ind->type
        ind->idx[i]=code2idx(sys,ind->code[i]);           //获载波频率的下标赋值ind->idx[i]
    ...
    for (i=0;i<NFREQ;i++) {//本层循环确定频率的优先级
        for (j=0,k=-1;j<n;j++) {//n为该系统观测值类型个数
            //ind->idx[= i: i从0开始每轮 + 1，idx[门越小，优先级越高 即code2freg_BDS()返回值越小，优先级越高
            if (ind->idx[j]==i&&ind->pri[j]&&(k<0||ind->pri[j]>ind->pri[k])) {
                k=j;//每一轮，选出优先级最高的频率(idx[i])+信道(pri[i]
            }
        }
        if (k<0) continue;
    ...
    for (i=0;i<NEXOBS;i++) {
        for (j=0;j<n;j++) {
            if (ind->code[j]&&ind->pri[j]&&ind->pos[j]<0) break;//每轮选出来最优的观测类型
        }
        if (j>=n) break;
...
{   
    ...
    /* read record */
    while (fgets(buff,MAXRNXLEN,fp)) {  //利用fgets()函数缓存一行数据，读取O文件的每一行
        //记录一个观测历元的有效性、时间和卫星数
        /* decode observation epoch */
                      ...
                      eph_t *eph)
{
    ...
    sys=satsys(sat,NULL);   //根据卫星编号来确定卫星所属的导航系统
    
    if (!(sys&(SYS_GPS|SYS_GAL|SYS_QZS|SYS_CMP|SYS_IRN))) {
        trace(3,"ephemeris error: invalid satellite sat=%2d\n",sat);        //  不支持则输出错误的信息，并返回
        return 0;
    }
    *eph=eph0;  //eph 初始化为空结构体 eph0
    
    eph->sat=sat;//存储卫星编号
    eph->toc=toc;//存储卫星时钟的参考时间（Toc）
    
    eph->f0=data[0];//卫星钟偏差
    eph->f1=data[1];//钟漂
    eph->f2=data[2];//钟漂的变化率
    ...
    //eph->A：轨道长半径的平方，SQR(data[10]) 计算出轨道半径
    //eph->e：轨道偏心率, eph->i0：轨道倾角; eph->OMG0：升交点赤经
    // eph->omg：近地点角距   eph->M0：平近点角    eph->deln：平均运动变化率;eph->OMGd 升交点赤经的变化率。
   //各种轨道修正参数，用于修正轨道模型的误差

    if (sys==SYS_GPS||sys==SYS_QZS) {   //卫星星历的标识符，用于跟踪星历版本。
        eph->iode=(int)data[ 3];      /* IODE */
        eph->iodc=(int)data[26];      /* IODC */
        eph->toes=     data[11];      /* Toe (s) in GPS week */
        eph->week=(int)data[21];      /* GPS week */
        eph->toe=adjweek(gpst2time(eph->week,data[11]),toc);//轨道历元（Toe）
        eph->ttr=adjweek(gpst2time(eph->week,data[27]),toc);//发送时刻（Ttr）的时间参数。
        
        eph->code=(int)data[20];      /* GPS: codes on L2 ch */
        eph->svh =(int)data[24];      /* SV health */
        eph->sva=uraindex(data[23]);  /* URA index (m->index) */    //精度指数
        eph->flag=(int)data[22];      /* GPS: L2 P data flag */ //L2.p码标识
        
        eph->tgd[0]=   data[25];      /* TGD */ //钟差修正参数（TGD，群时延差），用于修正卫星钟差
        if (sys==SYS_GPS) {
            eph->fit=data[28];        /* fit interval (h) */
        }
        else {
            eph->fit=data[28]==0.0?1.0:2.0; /* fit interval (0:1h,1:>2h) */ //适应时间
        }
    }
                       ...
                       int *type, eph_t *eph, geph_t *geph, seph_t *seph)
{
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            /* decode Toc field */
            // RINEX 导航文件的某一行中，提取卫星钟时间（Toc）。如果成功解析时间，它会将时间存储在 toc 变量中。如果解析失败，则输出调试信息，并返回错误。
            if (str2time(buff+sp,0,19,&toc)) {
                trace(2,"rinex nav toc error: %23.23s\n",buff);//读取卫星钟时间TOC
                return 0;
            }
            /* decode data fields */    //首行数据读3列，除了TOC还有3列
...
}//如果任意一种星历数量大于 0，函数返回 1，表示解析成功；否则返回 0，表示解析失败或无有效数据。
/* read RINEX clock ----------------------------------------------------------*/
                       ...
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    char tmpfile[1024];
    //fp: 文件指针，用于操作文件。
    //cstat : 状态码，用于检查解压缩的结果。
    //stat : 存储读取RINEX文件的状态码。
    //tmpfile : 存储解压后的临时文件名。

    trace(3,"readrnxfile: file=%s flag=%d index=%d\n",file,flag,index);
    //输出调试信息
    if (sta) init_sta(sta); //如果sta指针不为空，则调用init_sta函数初始化站点信息。
    
    /* uncompress file */   //解压文件file到tmpfile
    if ((cstat=rtk_uncompress(file,tmpfile))<0) {   //如果是z,gz或者zip的则会自动解压，调用rtk_uncompress函数解压文件，如果失败（返回值小于0），则打印错误信息并返回0。
        trace(2,"rinex file uncompact error: %s\n",file);
        return 0;
    }
   
    //以读的方式打开解压后的文件
    if (!(fp=fopen(cstat?tmpfile:file,"r"))) {//根据cstat的值判断解压是否成功，复数表示解压成功，r是只读模式
...
*
*-----------------------------------------------------------------------------*/
extern int readrnxt(const char *file, int rcv, gtime_t ts, gtime_t te,  //处理文件路径，赋值测站名
                    ...
                    sta_t *sta)
{
    ...
    /* if station name empty, set 4-char name from file head */     //如果测站名字为空，就给依据头文件自动赋4个字符的名字,作为站点名
    if (type=='O'&&sta) {
